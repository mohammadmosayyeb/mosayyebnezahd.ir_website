...
function saat() {
    ...
    document.querySelectorAll(".inu")[0].style.width = `${date.getHours() * 4.16}%`;
    document.querySelectorAll(".inu")[1].style.width = `${date.getMinutes() * 1.6}%`;
    document.querySelectorAll(".inu")[2].style.width = `${date.getSeconds() * 1.67}%`;
    ...
    dt.style.transform = `rotate(${sat + (dagiiige / 2)}deg)`;
    st.style.transform = `rotate(${minate}deg)`;
    sc.style.transform = `rotate(${seciuis}deg)`;

    seconderys = date.getSeconds();

    if (seconderys < 10) {
        seconderys = `0${seconderys}`
    }
    if (saaaaat < 10) {
        saaaaat = `0${saaaaat}`
    }
    ...
    document.querySelector(".Here").innerHTML = saaaaat;
    document.querySelector(".Hare").innerHTML = dagiiige;
    document.querySelector(".san").innerHTML = `${seconderys}"`;
...
function menu() {
    ...
    // document.querySelector(".personalize").style.display = "none";
}
function menuout() {
    var menu = document.querySelector(".person");
    menu.style.marginLeft = "-100vw";
    // document.querySelector(".personalize").style.display = "block";
...
document.querySelectorAll(".bot")[0].addEventListener("click" , ()=>{
    document.querySelectorAll(".minou")[0].value;
    console.log(document.querySelectorAll(".minou")[0].value);
    document.querySelector(".sec1").style.background = `#${document.querySelectorAll(".minou")[0].value}`;
})

document.querySelectorAll(".bot")[1].addEventListener("click" , ()=>{
    document.querySelectorAll(".minou")[1].value;
    console.log(document.querySelectorAll(".minou")[1].value);
    document.querySelector(".sec2").style.background = `#${document.querySelectorAll(".minou")[1].value}`;
})
